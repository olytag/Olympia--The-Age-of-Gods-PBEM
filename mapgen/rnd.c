...
void xMD5Update(struct xMD5Context *context, byte const *buf, long len);
 ...
 */
void
xMD5Update(struct xMD5Context *ctx, byte const *buf, long len)
{
	word32 t;
...
{
	long count = (long)(ctx->bytes[0] & 0x3f); /* Bytes in ctx->in */
...
void MD5(void *dest, void *orig, long len)
{
